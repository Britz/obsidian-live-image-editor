...
description: "Crop geometry & representation rework — 3-layer DOM + Bug 25 fix + crop serialization (Slices 1-4 done, uncommitted; crop-in-place Slice 5 deferred)"
metadata: 
  node_type: memory
  type: project
...
- **Bug 25 FIXED**: rotating a crop puts the rotate on the frame (centre pivot) → the img crop placement is byte-identical, no drift. CDP: crop vs crop+rotate90 had identical img transform, footprint swapped 240×180→180×240.
...
**Why:** Bug 25 + crop-in-place + crop serialization were ONE problem (crop geometry/representation); the orientation↔placement split + 3-layer solve all three.
