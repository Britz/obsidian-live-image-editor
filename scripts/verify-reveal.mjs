#!/usr/bin/env node
// LP REVEAL CLUSTER guard (Bug 29–31) — a runnable, read-the-real-DOM CDP check.
//
//   Bug 29: the toolbar reveal toggle renders the `<>` (Lucide "code") icon, not an eye.
//   Bug 30: a `<>` dismiss hides the WHOLE raw embed — the fake `![](…)` link AND the `{…}`.
//   Bug 31: the revealed `{…}` carries CM URL tokens (syntax highlighted), not plain text.
//
// Prereqs (CLAUDE.md → Live debugging): a dev build installed in examples/ + Obsidian running with
// the CDP relay. Run from the repo root:  node scripts/verify-reveal.mjs
...
const checks = [
  ["Bug 29: reveal icon is `<>` (lucide-code), not an eye", /lucide-code/.test(res.icon || "")],
  ["Bug 31: revealed {…} is syntax-highlighted (CM url tokens)", res.attrHighlighted === true],
  ["Bug 30: source revealed before dismiss (fake + attr shown)", res.before?.fake === "inline" && res.before?.attr === "inline"],
  ["Bug 30: dismiss hides the fake `![](…)` link", res.after?.fake === "none"],
  ["Bug 30: dismiss hides the `{…}` block", res.after?.attr === "none"],
];
