#!/usr/bin/env node
// §3 AD1 WRITE-PATH MATRIX — the runnable, load-bearing integration check (the Bug 33 guard).
//
// It drives EVERY toolbar/menu op in the running vault and READS THE REAL SOURCE `{…}` BACK,
// asserting the op's key landed — never assuming the DOM changed. It also runs the Bug 33
// duplicate-image case: an op on the SECOND embed of a repeated file must write to the SECOND
// line, not the first basename match. This is exactly the check Bug 33 slipped through (only the
// resize handle's `width`, via its separate posAtDOM path, persisted).
//
// Prereqs (CLAUDE.md → Live debugging): a dev build installed in examples/ and Obsidian running
// with the CDP relay. Run from the repo root:
//   node scripts/verify-write-path.mjs
...
// same-file images (the Bug 33 dup guard); the run drives each op through the real handlers, reads
...
const EVAL_RUN = `(async () => {
  ...
  try {
    ...
    // Bug 33 dup guard: rotate the SECOND landscape (line 10); the SECOND must change, the FIRST (line 7) must not.
    const dup = {};
...
const checks = [
  ...
  // Bug 33: the op on the SECOND duplicate-file image hits the SECOND line, not the first.
  ["Bug 33: 2nd image op writes the 2nd line", has(res.dup?.second, "rotate=")],
  ["Bug 33: the 1st image is untouched", !has(res.dup?.first, "{")],
];
