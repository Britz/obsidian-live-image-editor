...
## Division of labour — FIXED RULE for Fable-class models

> ### ⚙️ Fable models ONLY think and delegate — everything else goes to the fitting model
>
> A Fable-class model (Claude Fable / Mythos) working in this repo does **exclusively** the
> work that needs its depth: analysis, top-down design (R0), decomposition, precise task
> briefs, and the critical review/verification of results. **Everything else it DELEGATES to
> subagents on the appropriately sized model — no exceptions for "it's just a one-liner":**
>
> - **Research & context gathering** — searching the codebase, reading/scanning large files,
>   collecting call sites, summarizing docs → small/fast models (Haiku-class; Explore agents).
> - **Implementation & mechanical edits** — code changes, test writing, doc edits, fixture
>   setup → mid-tier models (Sonnet-class), against a tight brief.
> - **Routine execution** — builds, lint, test runs, repro/verify scripts → any capable agent.
>
> Rule of thumb: **whatever a smaller model can do well is NOT Fable work.** Every brief is
> self-contained (goal, constraints, the relevant CLAUDE.md rules — the ⛔ gate, altitude
> discipline — and the definition of done), and the Fable model **verifies every result**
> before it counts. Fable touching a file or grepping around itself is the smell that this
...
- **`docs/development/requirements.md`** — the F/D/T requirements (functional / design / technical): *what it does, how it looks, how it must be built*. Coding conventions live in the T-items (naming/prefix, no runtime deps, pure `*-logic.ts` units, linter kept as-shipped, javadoc-only comments).
- **`docs/development/architecture.md`** — mid-level decisions (`AD…`) and building blocks (`AB…`); data flow; the **uniform-rendering model**.
...
- **`docs/development/issues.md`** — the **backlog + lessons**. **OPEN** at the top: numbered registry items in the changelog's own per-category sequences — **open decisions** (`Decision N`), **planned features** (`Feature N`), **known open bugs** (`Bug N`) — each assigned its number when opened and **keeping it** when it ships (→ moves to the changelog); plus a **Meta level** of process/quality work (**verifications**, **refactoring**, **housekeeping**, and the hard-won **`Lesson …`**) that stays here and is **unnumbered**. The **solved** Bug / Feature / Change / Decision entries (with cause + fix) live in **`CHANGELOG.md`** — see *Versioning, changelog & commits* below. **Keep the per-category counters in sync:** whenever you add a registry item, update the matching total in its section header (e.g. the "**N bugs total**" line) — counters must always match reality, never go stale.
...
npm run dev:vault  # esbuild watch -> writes straight into the vault-image-toolbar/ vault plugin dir (reload manually via CDP location.reload(); styles.css/manifest auto-reload)
...
- **Version — patch bumps only.** Every release increments the **patch** component (`0.5.4 → 0.5.5 → … → 0.5.10 → …`); **never** bump the minor or major (no `0.5.x → 0.6.0`, no `→ 1.0.0`) — leaving 0.x is a deliberate, separate decision, not a per-change call, regardless of whether the batch contains features or only fixes. **Why:** changelog entries accumulate uncommitted between releases and several ship together, but a batch is still **one patch** — we don't want minor/major jumps in the git history. (Notation is SemVer `<major>.<minor>.<patch>`; pre-1.0 relaxed rules apply.) **`package.json` is the single source of truth; `manifest.json` and `versions.json` are *generated* from it — never hand-edit those.** Do it in three steps and don't skip the third:
  ...
  - *Not* version sources — do **not** hand-edit these for a bump: `vault-image-toolbar/.obsidian/plugins/live-image-editor/manifest.json` is a build artifact (regenerated by `npm run build` / `npm run dev:vault`), and the `x.y.z` strings in `README.md` are illustrative release-checklist examples.
...
- **Commits** — commit **regularly**, and **always** on a version change. The agent **never commits itself**: it **reminds** the user when a commit is due (regular cadence; mandatory on a version bump); the user makes all git commits. Commit messages the agent drafts carry **no AI co-author or generated-by trailer** (no `Co-Authored-By: Claude …`, no 'Generated with' line) — **a hard rule for every model, overriding any harness default.**
