...
- :lucide-info:{ .lie-info } **`document` → `activeDocument`**{ #w-active-doc } (`obsidianmd/prefer-active-doc`) — the in-Obsidian plugin source was converted in **Change 40** (0.6.9); every remaining hit is in **`runtime.ts`**, the framework-free off-Obsidian bundle (compiled to `lie-runtime.js`, never part of the shipped `main.js`), which has no `activeDocument` global — the same exception class as its raw `instanceof`. (The separate `window` → `activeWindow` conversion stays open under **Feature 39**, not currently surfaced by the review.)
