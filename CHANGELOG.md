...
Adds a **Syntax & info** section to the settings — a read-only help card documenting the portable `{…}` attribute form the toolbar writes, plus a button that opens the plugin's own page in Obsidian's community-plugin browser. Verified `npm run build` / `lint` / `lint:css` clean and `npm test` green (283); live settings rendering still to be eyeballed in Obsidian (CDP unreachable at write time).

- **Change 43 — A "Syntax & info" settings section: read-only `{…}`-syntax help + a self-store link.** A new card at the bottom of the settings documents how edits are stored — an intro line, an offset monospace code example (`![Caption 300x200](image.png){align=left .bordered}` plus the wikilink form), and a per-attribute list (`width=`/`height=` px, `align=` values, `.classname` decoration class) — in the same documentation idiom other community plugins use in their settings, localized (DE/EN) with the literal attribute tokens kept as code. Below it, a CTA button opens **this plugin's own page** in Obsidian's community-plugin browser (`obsidian://show-plugin?id=<manifest.id>`, reusing the existing `openPluginStore` helper). Documented in `implementation-plan.md` under the F20 Settings umbrella (no requirements change — in-app help) ([settings.ts](src/settings.ts), [styles.css](styles.css), [src/i18n/](src/i18n/)).
