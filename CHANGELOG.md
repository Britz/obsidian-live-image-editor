...
Crop-overflow rework — Obsidian 1.12.7 raised the specificity of its block-widget `contain:paint`, which silently defeated the crop editor's `contain:none` lift, so the croppable surround and the resize frame/corner were clipped at the cut window. Replaced the brittle lift with a body-portal model that stops fighting containment. Verified live via CDP on 1.12.7 (`verify-resize-affordance` veil-portal canary + a new optical pixel check, `verify-crop` 19/19, `verify-crop-pan` 11/11, `verify-crop-teardown` all exit paths) plus `npm test` green (283), `npm run build` / `lint` / `lint:css` clean and `lint:obsidian` 0 errors.
