...
Crop-overflow rework — Obsidian 1.12.7 raised the specificity of its block-widget `contain:paint`, which silently defeated the crop editor's `contain:none` lift, so the croppable surround and the resize frame/corner were clipped at the cut window. Replaced the brittle lift with a body-portal model that stops fighting containment. This release also lands a **black-box / optical + functional CDP test layer** (Change 41) — pixel + pointer-hit-test checks that observe the painted result instead of reading CSS properties, with a one-command runner — plus an honesty pass removing false-greens and un-runnable false-reds from the existing checks (Change 42); exactly the layer whose absence let this regression ship unseen. Verified live via CDP on 1.12.7 (`verify-resize-affordance` veil-portal canary + a new optical pixel check, `verify-crop` 19/19, `verify-crop-pan` 11/11, `verify-crop-teardown` all exit paths) plus `npm test` green (283), `npm run build` / `lint` / `lint:css` clean and `lint:obsidian` 0 errors.
