...
// live preview equals the committed render and a release never drifts (Bug 32 A). In-place the
// editor's scale-1 baseline IS the cut-frame width, so parse∘serialize is the identity.
describe("parsePlacement ∘ toCropResult round-trip (Bug 32 A — no centre/top-left drift)", () => {
