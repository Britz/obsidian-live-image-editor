...
//
// Bug 33 (the bare-key WRITE PATH) is FIXED and guarded HERE in the suite by the §2.8 per-op
...
// Bug 32 — crop editor migrated to the live 3-layer model. The PURE, autonomously-verifiable half:
...
// so these are CDP scripts, not vitest units (T-L6, like verify-write-path.mjs).
describe("Bug 32 G — a width edit preserves the crop (both write paths keep transform=/aspect-ratio=)", () => {
...
// Bug 29–31 — the LP reveal / source-rendering cluster — SOLVED (see issues.md → "Resolved by the
// LP reveal cluster fix"). The LIVE-DOM facts are obsidian/CM-coupled and pinned by the read-the-
// real-DOM CDP guard `scripts/verify-reveal.mjs` (Bug 29: the toolbar reveal SVG is `lucide-code`,
// not an eye; Bug 30: a `<>` dismiss computes `display:none` on BOTH `.lie-fake-link` AND
// `.lie-attr`; Bug 31: the revealed `{…}` carries `cm-url` tokens) — structural checks, not vitest
// units (T-L6, like verify-write-path/verify-crop). What IS purely verifiable is pinned below: the
// Bug-30 dismiss/auto-clear STATE MACHINE (`reduceReveal`) and the Bug-31 highlight CLASS invariant.

describe("Bug 30 — the `<>` dismiss / auto-clear state machine (reduceReveal — auto mode is default)", () => {
...
describe("Bug 31 — the revealed {…} highlight CLASS invariant (live DOM = verify-reveal.mjs)", () => {
