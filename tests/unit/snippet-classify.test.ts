...
describe("parseImgRules", () => {
  it("extracts .NAME rules with normalized bodies (the class rides the outer, Decision 28)", () => {
    const rules = parseImgRules(".rounded { border-radius: 8px; }\n.shadow{box-shadow:0 0 1px}");
    expect(rules.get("rounded")).toBe("border-radius:8px");
    expect(rules.get("shadow")).toBe("box-shadow:0 0 1px");
  });

  it("ignores the auxiliary compound pixel rule (.circle img, img.circle)", () => {
    const rules = parseImgRules(".circle { border-radius: 50%; }\n.circle img, img.circle { object-fit: cover; }");
    expect(rules.get("circle")).toBe("border-radius:50%");
    expect(rules.size).toBe(1); // the compound .circle img / img.circle rule is NOT a class entry
  });

  it("still accepts the legacy img.NAME form (a pre-Decision-28 installed copy migrates)", () => {
    expect(parseImgRules("img.rounded { border-radius: 8px; }").get("rounded")).toBe("border-radius:8px");
  });

  it("treats whitespace/trailing-semicolon differences as identical (normalized)", () => {
    expect(parseImgRules(".a{b:1;}").get("a")).toBe(parseImgRules(".a {  b: 1  }").get("a"));
  });

  it("ignores rules inside comments (Bug 34 family)", () => {
    const rules = parseImgRules("/* .fake { x: 1 } */ .real { y: 2; }");
...
describe("classifyBundledFile", () => {
  ...
  it("detects a changed body and a deleted class against the shipped default", () => {
    const edited = BUNDLED_SNIPPET_CSS
      .replace(".rounded { border-radius: 8px; }", ".rounded { border-radius: 20px; }")
      .replace(".shadow { box-shadow: 0 4px 14px color-mix(in srgb, var(--text-normal) 70%, transparent); }", "");
...
describe("restoreClassInCss", () => {
  it("replaces a changed rule in place", () => {
    ...
    expect(restored).toContain(".rounded { border-radius: 8px; }");
    expect(restored).not.toContain("99px");
  });

  it("re-appends a deleted rule", () => {
    const edited = BUNDLED_SNIPPET_CSS.replace(".circle { border-radius: 50%; aspect-ratio: 1; }", "");
...
describe("isBundledFileModified", () => {
  ...
  it("is true when a shipped class is changed, deleted, or an extra class is added", () => {
    ...
    expect(isBundledFileModified(BUNDLED_SNIPPET_CSS.replace(".circle { border-radius: 50%; aspect-ratio: 1; }", ""), BUNDLED_SNIPPET_CSS)).toBe(true);
    expect(isBundledFileModified(BUNDLED_SNIPPET_CSS + "\n.custom { color: red; }", BUNDLED_SNIPPET_CSS)).toBe(true);
  });
});
